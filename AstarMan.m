function [NbNode,Chemin]=AstarMan(NodeList,Map2d,RootNode,TargetNode)
%open=[];
closed=[];
...
open =[RootNode norm([target_x-act_x target_y-act_y],1) 0]
closed=[]
while ~isempty(open)
%     if ~isempty(open)
...
%     end
    ...
    for i=1:length(NodeList(Node(1)).Neigh)
      x=find(closed(:,1)==NodeList(Node(1)).Neigh(i));
        if  isempty(x)
	% cout_du_chemin ajoute au cout du noeud parent le cout pour aller au nouveau noeud        
       cout_du_chemin = Node(3)+NodeList(Node(1)).K(i)
        ...
        cout_total = cout_du_chemin + norm([target_x-act_x target_y-act_y],1)
